...
namespace ECGVisualizer
{
    class ECGpeaks
    {
        ...
        public static bool TposOneg2(List<double> ecgSignal, List<double> puntosS, int ventanaMax, int ventanaMin, int hercios, double ruido)
        {
            ...
            double segSTmin = (ventanaMin * 1.0) - (ventanaMin * ruido);//ventana;
            double segSTmax = ((ventanaMax * 1.0) + (ventanaMax * ruido))*1.5;//ventana * 3.0;
            int numNegs = 0, numPos = 0;

            foreach (double puntoS in puntosS)
            {
                int ini = (int)((puntoS * hercios) + segSTmin) > (ecgSignal.Count - 1)? (ecgSignal.Count - 1): (int)((puntoS * hercios) + segSTmin);// (int)((int)((puntoS*hercios) + segSTmin)) > (ecgSignal.Count - 1) ? (ecgSignal.Count - 1) : ((int)(puntoS*hercios) + segSTmin);
            ...
            //sum = sum / count;
            if (numNegs > numPos)// - sum) < Math.Abs(sum - min))
                pos = false;
        ...
        public static bool PposOneg(List<double> ecgSignal, List<double> puntosQ, int ventanaMax, int ventanaMin, double ruido)
        {
            ...
            double sum = 0.0;
            double max = -500, min = 500;
            int segPQmin = (int) ((ventanaMin * 0.8) - (ventanaMin * ruido));//Convert.ToInt32(0.3 * hercios);
            int segPQmax = (int) (ventanaMax * 1.2 + (ventanaMax * ruido));// * 2;//Convert.ToInt32(0.5 * hercios);
        ...
        public static List<double> DetectarPicosR2(List<double> ecgSignal, int ventanaAncho, int retraso, int hercios, double ruido)
        {
            ...
            if (normal)
            {
                ...
                for (int i = indexMax + retraso; i < ecgSignal.Count; i++)
                {
                    ...
                    for (int j = i; j<fin; j++)//j < i + ventanaAncho && (i + ventanaAncho) < ecgSignal.Count; j++)
                    {
        ...
        public static List<double> DetectarPicosQ2(List<double> ecgSignal, List<double> picosR, int ventanaAnchoMax, int ventanaAnchoMin, double umbralPicoP, int hercios, double ruido)
        {
            ...
            if (normal)
            {
                foreach (double picoR in picosR)
                {
                    ...
                    ini = indexMin;// - (int)(indexMin * umbralPicoP);
                    //ini = (indexMin - QRsegMin)<0? 0: (indexMin - QRsegMin);
                    ...
                    for (int i = ini; i >= fin && i >= 0; i--)
                    {
                        ...
                        if (anterior>ecgSignal[i])// && ecgSignal[i] < min)
                        {
                            anterior = min;
                            min = ecgSignal[i];
                            indexMin = i;
            ...
            else
            {
                foreach (double picoR in picosR)
                {
                    ...
                    ini = indexMax;// - (int)(indexMax * umbralPicoP);
                    fin = (ini - QRsegMax - 1) < 0 ? 0:ini - QRsegMax - 1;
                    for (int i = ini; i >= fin && i >= 0; i--)
                    {
                        /*if (ecgSignal[i] > max)
                        ...
                        if (anterior < ecgSignal[i])// && ecgSignal[i] > max)
                        {
                            anterior = max;
                            max = ecgSignal[i];
                            indexMax = i;
        ...
        public static List<double> DetectarPicosS2(List<double> ecgSignal, List<double> picosR, int ventanaAnchoMax, int ventanaAnchoMin, double umbralPicoP, int hercios, double ruido)
        {
            ...
            if (normal)
            {
                ...
                {
                    indexMin = (int)(picoR * hercios) + 1;
                    ini = (indexMin+1) > (ecgSignal.Count - 1) ? (ecgSignal.Count - 1) : (indexMin + 1);// + (int)(picoR * umbralPicoP);
                    min = ecgSignal[ini];
                    anterior = ecgSignal[ini-1];
                    //ini = (indexMin + RSsegMin) > (ecgSignal.Count - 1) ? (ecgSignal.Count - 1) : (indexMin + RSsegMin);// + (int)(picoR * umbralPicoP);
            ...
            else
            {
                ...
                {
                    indexMax = (int)(picoR * hercios) + 1;
                    ini = (indexMax+1) > (ecgSignal.Count - 1) ? (ecgSignal.Count - 1) : (indexMax + 1); ;// - (int)(indexMax * umbralPicoP);
                    max = ecgSignal[ini];
                    anterior = ecgSignal[ini - 1];
                    fin = (ini + RSsegMax + 1) > (ecgSignal.Count - 1) ? (ecgSignal.Count - 1) : (ini + RSsegMax + 1);
                    for (int i = ini; i <= fin && i < ecgSignal.Count; i++)
         ...
         public static List<double> DetectarPicosT1(List<double> ecgSignal, List<double> picosS, int ventanaAnchoMax, int ventanaAnchoMin, int hercios, double ruido, int window)
        {
            ...
            int segmentSTmin = (int)((ventanaAnchoMin * 1.0) - (ventanaAnchoMin * ruido));//ventanaAncho;//(int)(ventanaAncho/2.0);
            int segmentSTmax = (int)((ventanaAnchoMax * 1.0) + (ventanaAnchoMax * ruido));//(int)(ventanaAncho*3);
        ...
        public static List<double> DetectarPicosP1(List<double> ecgSignal, List<double> picosQ, int ventanaAnchoMax, int ventanaAnchoMin, int hercios, double ruido, int window)
        {
            ...
            //int segmentPQmin = Convert.ToInt32(0.1 * hertzFiltered);
            //int segmentPQmax = Convert.ToInt32(0.3 * hertzFiltered);
            ...
            else
            {
                foreach (double picoQ in picosQ)
                {
                    ...
                    //indexMin = (picoQ - segmentPQmin)>=0? picoQ - segmentPQmin:0;)
                    indexMax = (int)((picoQ*hercios)-PQsegMin) >= 0 ? (int)((picoQ * hercios) - PQsegMin) : 0;
                    max = ecgSignal[indexMax];
                    ini = indexMax;
                    fin = ((int)(picoQ * hercios) - PQsegMax) >= 0 ? (int)(picoQ * hercios) - PQsegMax : 0;
                    //fin = (int)((picoQ-PQsegMax) * hercios) >= 0 ? (int)((picoQ - PQsegMax) * hercios) : 0;
