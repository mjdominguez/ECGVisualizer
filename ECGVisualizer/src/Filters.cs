...
namespace ECGVisualizer
{
    class Filters
    {
        ...
        public static List<double> AmplitudeModification(List<double> signal, double factor)
        {
            List<double> result = new List<double>();

            for (int i = 0; i < signal.Count; i++)
            {
                double amplification = signal[i] * factor;
                result.Add(amplification);
            }

            return result;
        }

        public static List<double> TimingModification(List<double> signal, double factor)
        {
            List<double> result = new List<double>();

            for (double i = 0; i < signal.Count; i=i+factor)
            {
                double amplification = signal[Convert.ToInt32(Math.Truncate(i))];
                result.Add(amplification);
            }
            return result;
        }
